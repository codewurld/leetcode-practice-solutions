...
function isFirstComeFirstServed(takeOutOrders, dineInOrders, servedOrders) {
    ...
    for(let i = 0; i < servedOrders.length; i++){
      ...
      if(takeOutOrdersIndex <= takeOutOrdersMaxIndex && currentServedOrder === takeOutOrders[takeOutOrdersIndex]){
        takeOutOrdersIndex++;
      }
      
      else if (dineInOrdersIndex <= dineInOrdersMaxIndex && currentServedOrder === dineInOrders[dineInOrdersIndex]){
        dineInOrdersIndex++;
      }
       else {
         return false;
